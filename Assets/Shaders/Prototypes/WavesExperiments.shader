Shader "Custom/WavesExperiments"
{
    Properties
    {
        ...
        _WaveAmp ("Wave Amplitude", Range(0,.5)) = 1
    ...
    SubShader
    {
        ...
        float GetWave( float2 uv ) {
            float2 uvsCentered = uv * 2 - .5;
            float2 distanceToDropOne = distance(uvsCentered, _DropOne.xy);
            // float2 distanceToDropOne = distance(uv, _DropOne.xy);
            // return saturate(.5-distanceToDropOne);

            float wave1 = cos( (distanceToDropOne - _Time.y * _Speed ) * TAU * _WaveCount ) * 0.5 + 0.5;
            wave1 *= saturate(1 - distanceToDropOne);
